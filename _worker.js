...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	if (!config_JSON.优选订阅生成.自定义优选) {
		config_JSON.优选订阅生成.自定义优选 = 默认配置JSON.优选订阅生成.自定义优选;
	}
	delete config_JSON.订阅转换配置; // 已改为原生生成，不再需要
...
function 生成原生ClashYAML(优选IP列表, 其他节点文本, config, 协议类型) {
	...
	for (const link of allLinks) {
		try {
			...
			const name = 格式化节点名称(tag);
			proxies.push({ name, type:'vless', server:u.hostname.replace(/^\[|\]$/g,''), port:parseInt(u.port)||443, uuid, tls:true, 'skip-cert-verify':!!config.跳过证书验证, network:'ws', 'ws-opts':{ path, headers:{Host:host} }, 'client-fingerprint':fp, servername:host });
	...
	for (const addr of 优选IP列表) {
		...
		if (!m) continue;
		const name = 格式化节点名称(m[3] || m[1]);
		proxies.push({ name, type:'vless', server:m[1].replace(/^\[|\]$/g,''), port:parseInt(m[2])||443, uuid, tls:true, 'skip-cert-verify':!!config.跳过证书验证, network:'ws', 'ws-opts':{ path, headers:{Host:host} }, 'client-fingerprint':fp, servername:host });
		proxyNames.push(name);
	}
	...
	for (let i = 0; i < proxyNames.length; i++) {
		seen[proxyNames[i]] = (seen[proxyNames[i]]||0)+1;
		if (seen[proxyNames[i]] > 1) { proxyNames[i] += ' '+seen[proxyNames[i]]; proxies[i].name = proxyNames[i]; }
	}

	const names = proxyNames.map(n => '      - "'+n.replace(/"/g,'\\"')+'"').join('\n');
...
function 生成原生SingboxJSON(优选IP列表, 其他节点文本, config, 协议类型) {
	const uuid = config.UUID, host = config.HOST, path = config.完整节点路径 || '/?ed=2560', fp = config.Fingerprint || 'chrome';
	const outbounds = [], tags = [];

	const allLinks = 其他节点文本 ? 其他节点文本.split('\n').filter(l => l.trim()) : [];
	for (const link of allLinks) {
		try {
			const u = new URL(link.replace('vless://','http://'));
			const name = 格式化节点名称(decodeURIComponent(u.hash.slice(1))||u.hostname);
	...
	for (const addr of 优选IP列表) {
		...
		if (!m) continue;
		const name = 格式化节点名称(m[3]||m[1]);
		outbounds.push({type:'vless',tag:name,server:m[1].replace(/^\[|\]$/g,''),server_port:parseInt(m[2])||443,uuid,tls:{enabled:true,server_name:host,utls:{enabled:true,fingerprint:fp}},transport:{type:'ws',path,headers:{Host:host}}});
		tags.push(name);
	}
	const seen = {};
	for (let i=0;i<tags.length;i++){seen[tags[i]]=(seen[tags[i]]||0)+1;if(seen[tags[i]]>1){tags[i]+=' '+seen[tags[i]];outbounds[i].tag=tags[i];}}

	return JSON.stringify({
		log:{level:'info',timestamp:true},
		dns:{servers:[{tag:'remote',address:'https://dns.google/dns-query',detour:'select'},{tag:'local',address:'https://dns.alidns.com/dns-query',detour:'direct'}],rules:[{geosite:['cn'],server:'local'},{geosite:['geolocation-!cn'],server:'remote'}],strategy:'prefer_ipv4'},
		inbounds:[{type:'mixed',tag:'mixed-in',listen:'127.0.0.1',listen_port:7890}],
		outbounds:[
			{type:'selector',tag:'select',outbounds:['direct',...tags]},
			...outbounds,
			{type:'direct',tag:'direct'},
			{type:'block',tag:'block'},
			{type:'dns',tag:'dns-out'}
		],
		route:{rules:[{protocol:'dns',outbound:'dns-out'},{geosite:['cn'],geoip:['cn','private'],outbound:'direct'},{geosite:['geolocation-!cn'],outbound:'select'}],final:'select',auto_detect_interface:true}
	},null,2);
}
